...
#ifndef CHARTCTRL_EXPORT
#define CHARTCTRL_EXPORT
#endif
...
BEGIN_MESSAGE_MAP(CChartCtrl, CWnd)
	...
	ON_COMMAND(ID_POP_ZOOM, &CChartCtrl::OnPopZoom)
	ON_COMMAND(ID_POP_PAN, &CChartCtrl::OnPopPan)
	ON_COMMAND(ID_POP_NONE, &CChartCtrl::OnPopNone)

	ON_UPDATE_COMMAND_UI(ID_POP_ZOOM, &CChartCtrl::OnUpdatePopZoom)
	ON_UPDATE_COMMAND_UI(ID_POP_PAN, &CChartCtrl::OnUpdatePopPan)
	ON_UPDATE_COMMAND_UI(ID_POP_NONE, &CChartCtrl::OnUpdatePopNone)
...
__declspec(dllexport) void CChartCtrl::RefreshCtrl()
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState()); 
...
void CChartCtrl::OnMouseMove(UINT nFlags, CPoint point) 
{
	//AFX_MANAGE_STATE(AfxGetStaticModuleState()); 
	//改
	if (m_bLMouseDown && m_bPanEnabled)
	{
...
//LButtonZoom
//void CChartCtrl::OnLButtonDownZoom(UINT nFlags, CPoint point)  
//{ 
//	if (m_bZoomEnabled)
//	{
//		m_bLMouseDown = true;
...
void CChartCtrl::OnLButtonUpZoom(UINT nFlags, CPoint point)  
{ 
...
//右键菜单弹出
void CChartCtrl::OnRButtonMenu(UINT nFlags, CPoint point)  
{ 
...
void CChartCtrl::OnUpdatePopZoom(CCmdUI *pCmdUI)
{
	if(m_bZoomEnabled)
...
void CChartCtrl::OnUpdatePopPan(CCmdUI *pCmdUI)
{
	if(m_bPanEnabled)
		pCmdUI->SetCheck(true);
	else	
...
void CChartCtrl::OnUpdatePopNone(CCmdUI *pCmdUI)
{
	if(m_bZoomEnabled||m_bPanEnabled)
		pCmdUI->SetCheck(false);
	else	
		pCmdUI->SetCheck(true);
}

void CChartCtrl::OnPopZoom()
{
	SetZoomEnabled(true);
	SetPanEnabled(false);
}

void CChartCtrl::OnPopPan()
{
	SetZoomEnabled(false);
	SetPanEnabled(true);
}

void CChartCtrl::OnPopNone()
{
	SetZoomEnabled(false);
	SetPanEnabled(false);
}

void CChartCtrl::OnInitMenuPopup(CMenu   *pPopupMenu,   UINT   nIndex,BOOL   bSysMenu)  
{  
