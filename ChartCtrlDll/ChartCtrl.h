 ...
 */
	...
	#define CHARTCTRL_EXPORT_CLASS _declspec(dllimport)
#else
	#define CHARTCTRL_EXPORT_CLASS _declspec(dllexport)
#endif
...
**/
class CHARTCTRL_EXPORT_CLASS CChartCtrl : public CWnd
{
...
private:
	//右键菜单弹出部分
	void OnRButtonMenu(UINT nFlags, CPoint point) ;
	void OnLButtonUpZoom(UINT nFlags, CPoint point);
public:
	//设置坐标范围
	void SetAxis(double XFrom=0,double XTo=10,double YFrom=0,double YTo=10);
...
public:
	//拖拽相关选项
	afx_msg void OnUpdatePopZoom(CCmdUI *pCmdUI);
	afx_msg void OnUpdatePopPan(CCmdUI *pCmdUI);
	afx_msg void OnUpdatePopNone(CCmdUI *pCmdUI);
	
	afx_msg void OnPopZoom();
	afx_msg void OnPopPan();
	afx_msg void OnPopNone();
